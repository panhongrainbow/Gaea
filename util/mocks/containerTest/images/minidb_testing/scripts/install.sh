...
database_version="10.5"         # 数据库版本; database version
database_repo_key="https://mariadb.org/mariadb_release_signing_key.asc"
                                # 数据库 repo 密钥; database repo key
database_repo="deb [arch=amd64,i386,arm64,ppc64el] https://mirrors.aliyun.com/mariadb/repo/10.5/debian bullseye main"
...
panic "$(apt_update 10 && echo "continue" || echo "panic")"
                                # 更新 apt 源; update apt source
apt_add_repo "$database_repo_key" "$database_repo" 10
                                # 添加数据库 apt 源; add apt database source
panic "$(apt_update 10 && echo "continue" || echo "panic")"
                                # 更新 apt 源; update apt source
panic "$(check_or_correct_database_version "${database_type}" "${database_version}" "check" && echo "continue" || echo "panic")"
                                # 检查数据库版本; check database version
# <<<<<<<< 环境初始化设置 environment initialize <<<<<<<<<<

# >>>>>>>> 设定数据库 set mariadb >>>>>>>>>>
panic "$(apt_install "${deb_package}" 10  && echo "continue" || echo "panic")"
                                # 安装 mariadb 数据库; install mariadb database
...
check_or_correct_database_version "${database_type}" "${database_version}" "correct"
                                # 检查数据库版本; check database version
# <<<<<<<< 设定数据库 set mariadb <<<<<<<<<<
